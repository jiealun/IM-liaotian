...
function setupEmojiPicker() {
  ...
  // Build tabs
  tabs.innerHTML = categories.map(([icon], i) =>
    `<button class="emoji-tab ${i===0?'active':''}" data-idx="${i}">${icon}</button>`
  ).join('');

  function showCategory(idx) {
    tabs.querySelectorAll('.emoji-tab').forEach((t,i) =>
    ...
    grid.querySelectorAll('.emoji-item').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        input.value = val.slice(0, start) + btn.textContent + val.slice(end);
  ...
  tabs.addEventListener('click', (e) => {
    ...
    if (btn) showCategory(parseInt(btn.dataset.idx));
  });
